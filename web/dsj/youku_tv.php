...
if(isset ($_GET['page'])){
    ...
    foreach ($id[1] as $reload)
    {
       ...
       $d = count($b[1]);
       for($m=0;$m<$d;$m++){
       $xml .='<m type="youku" src="' . $b[1][$m] . '" label="' . $c[1][$m] . '" />'."\n";
}}}
else{
        $xml.=page(1);
}
...
function page($page){
        $a=m_v('http://list.youku.com/category/show/c_97_a_大陆_s_1_d_1_p_'.$page.'.html');
        ...
        $xml .='<m label="'.$b[2][$index].'-'.$g[1][$index].'" list_src="'.URL.'?id='.$id.'" />'."\n";
}
if ($page==1){
                for ($i=2;$i<=34;$i++){
